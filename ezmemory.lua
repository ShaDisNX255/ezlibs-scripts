...
local function get_item_get_duration_seconds(player_id)
    local fallback_duration = 0.34

    local ok, parsed = pcall(function()
        return avatar_utils.parse_player_animation(
            player_id,
            "sheet"
        )
    end)
    ...
    if not ok or not parsed then
        print(
            "[ezmemory][item_get] animation parse failed, using fallback duration"
        )
        return fallback_duration
    end

    local animations =
        parsed.animations or parsed["animations"]

    local item_get =
        animations and animations["ITEM_GET"]

    local duration =
        tonumber(
            item_get and
            item_get.total_duration_ms
        )

    if not duration then
        print(
            "[ezmemory][item_get] ITEM_GET duration missing, using fallback duration"
        )
        return fallback_duration
    end
...
function ezmemory.on_tick(delta_time)
    ...
    for player_id, state in
        pairs(pending_item_get_anims)
    do
        if
            Net.is_player and
...
function ezmemory.set_direction_anim(
    player_id,
    direction
)
    pending_item_get_anims[player_id] = nil
