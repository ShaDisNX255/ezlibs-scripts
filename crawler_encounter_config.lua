...
config.package_path =
    "/server/assets/ezlibs-assets/ezencounters/ezencounters.zip"
...
-- receives for its personal encounter pool.
config.area_pool_size = {
    easy = 7,
    medium = 7,
    hard = 7,

    -- Boss territory can spawn up to 8 normal viruses,
    -- so it needs at least 8 entries available.
    boss = 8,
}


-- Number of viruses in a normal encounter.
config.enemy_count = {
    easy = {
        min = 2,
        max = 3,
...
-- Only used in boss territory.
--
-- 0.30 = 30% chance that a random encounter becomes
-- a boss encounter instead of a normal 4-8-virus swarm.
config.boss_chance =
    0.30
...
-- ============================================================
-- BOSS TERRITORY BOSS POOL
-- ============================================================
--
-- First pass uses base boss ranks.
-- We can add V2/V3/EX/SP weighting after the system itself
-- is proven stable.
