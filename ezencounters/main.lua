...
local battle_reward_types = { money = 0, hp = 2, bugfrags = 3, }
...
local get_enemy_reward_table = function(enemy)
    ...
    local drops = battle_rewards.drops or
        battle_rewards

    local enemy_rewards = drops[enemy.name]
...
local get_money_value = function(reward_table, difficulty, tier)
    if reward_table and type(reward_table.money) == "table" then
        local value = tonumber(reward_table.money[tier])
    ...
    local money_cfg = battle_rewards and
        battle_rewards.money

    local difficulty_cfg = money_cfg and
        money_cfg[difficulty]

    return math.floor(tonumber(difficulty_cfg and difficulty_cfg[tier]) or 0)
end

local get_chip_chance = function(reward_table, difficulty, tier)
    if reward_table and type(reward_table.chip_chances) == "table" then
        local override = tonumber(reward_table.chip_chances[tier])
    ...
    local chances = battle_rewards and
        battle_rewards.chip_chances

    local difficulty_chances = chances and
        chances[difficulty]

    return tonumber(difficulty_chances and difficulty_chances[tier]) or 0
end
...
local get_battle_reward = function(player_id, encounter_info, stats, persistent_health)
    ...
    local source_enemy = pick_reward_enemy(encounter_info)

    if not source_enemy then
        return nil, 0, nil
    end

    local reward_table = get_enemy_reward_table(source_enemy)

    local difficulty = (reward_table and reward_table.difficulty) or
    ...
    local recovery = reward_table and
        reward_table.low_hp_recovery

    if not recovery then
        recovery = battle_rewards.low_hp_recovery
    end

    local recovery_reward = nil
    ...
    if persistent_health and type(recovery) == "table" and max_health > 0 then
        ...
        if value > 0 and health < max_health * threshold then
            recovery_reward = { type = "hp", value = value, }
        end
    end

    local function finish_reward(primary_reward, delay_ticks, post_send_chip_key)
        return
            primary_reward,
            delay_ticks or 0,
            recovery_reward,
    ...
    local money = get_money_value(reward_table, difficulty, tier)

    local chip_key = reward_table and
        (reward_table.chip or reward_table.card)
    ...
    if not chip_key then
        if money > 0 then
            return finish_reward({ type = "money", value = money, }, 0)
        end

        return finish_reward(nil, 0)
    end

    local chip_chance = get_chip_chance(reward_table, difficulty, tier)
    ...
    if crawler_whitelist.player_has_card_unlocked(player_id, chip_key) then
        if money > 0 then
            return finish_reward({ type = "money", value = money, }, 0)
        end
    ...
    local ready, reason, _, card_reward =
        crawler_whitelist.prepare_card_for_battle_reward(player_id, chip_key)

    if ready and card_reward then
        print(
...
if #native_rewards > 0 then
    for index, reward in ipairs(native_rewards) do
        print(
...
local queue_post_reward_unlock = function(player_id, card_key)
        pending_post_reward_unlocks[player_id] = { ticks = 1, card_key = card_key, }
    end

local queue_battle_reward_packet = function(player_id, rewards, stats, persistent_health, ticks)
        pending_battle_reward_packets[
            player_id
        ] = {
            ticks = math.max(1, math.floor(tonumber(ticks) or 1)),
            rewards = rewards,
            stats = { health = stats and stats.health or 0, },
...
Net:on("tick", function()
    for player_id, packet in pairs(pending_battle_reward_packets) do
        ...
        if packet.ticks <= 0 then
            if Net.is_player(player_id) then
                send_battle_rewards(player_id, packet.rewards, packet.stats, packet.persistent_health)
            end
    ...
    for player_id, packet in pairs(pending_post_reward_unlocks) do
        ...
        if packet.ticks <= 0 then
            if Net.is_player(player_id) then
                print("[ezencounters][POST REWARD UNLOCK] " .. tostring(packet.card_key))

                crawler_whitelist.commit_card_after_battle_reward(player_id, packet.card_key)
            end
...
local function crawler_shuffle(list)
    for i = #list, 2, -1 do
...
local function crawler_pick_distinct(source, count)
    local copy = {}
...
local function crawler_get_master_pool(difficulty)
    ...
    if difficulty == "hard" or difficulty == "boss" then
...
local function crawler_create_area_pool(difficulty)
    local master_pool = crawler_get_master_pool(difficulty)

    if not master_pool or #master_pool == 0 then
        return {}
    end

    local pool_size = crawler_encounter_config.area_pool_size[difficulty] or 7

    return crawler_pick_distinct(master_pool, pool_size)
end

local function crawler_build_positions(enemy_count)
    local positions = crawler_blank_positions()

    local cells = crawler_pick_distinct(CRAWLER_ENEMY_CELLS, enemy_count)
...
local function crawler_pool_to_string(pool)
    ...
    for _, enemy in ipairs(pool or {}) do
        parts[#parts + 1] = tostring(enemy.name) ..
            "[" ..
            tostring(enemy.rank) ..
            "]"
    end
...
local function get_or_create_crawler_area_table(area_id)
    if not area_id then
        return nil
    end
    ...
    local room_type = Net.get_area_custom_property(area_id, "dungeon_room_type")
    ...
    local difficulty = Net.get_area_custom_property(area_id, "dungeon_difficulty")

    if difficulty ~= "easy" and difficulty ~= "medium" and difficulty ~= "hard" and difficulty ~= "boss" then
        return nil
    end
    ...
    if reward_tier ~= "easy" and reward_tier ~= "medium" and reward_tier ~= "hard" then
        reward_tier = difficulty == "boss"
                and "hard"
                or difficulty
    end

    local area_pool = crawler_create_area_pool(difficulty)
    ...
    local encounter_table = {
        ...
        minimum_steps_before_encounter = crawler_encounter_config.minimum_steps_before_encounter,
        encounter_chance_per_step = crawler_encounter_config.encounter_chance_per_step,
        crawler_random = true,
    ...
    print(
        ...
        tostring(area_id) ..
        ": " ..
        crawler_pool_to_string(area_pool)
    )

    return encounter_table
end

local function build_crawler_random_encounter(area_id, encounter_table)
    local difficulty = encounter_table.crawler_difficulty

    -- --------------------------------------------------------
    -- BOSS ROLL
    -- --------------------------------------------------------

    if difficulty == "boss" and math.random() < crawler_encounter_config.boss_chance then
        local boss_pool = crawler_encounter_config.boss_pool

        if boss_pool and #boss_pool > 0 then
            local boss = boss_pool[math.random(#boss_pool)]
    ...
    local count_cfg = crawler_encounter_config.enemy_count[difficulty]

    if not count_cfg then
        return nil
    end

    local enemy_count = math.random(count_cfg.min, count_cfg.max)

    enemy_count = math.min(enemy_count, #encounter_table.crawler_pool, #CRAWLER_ENEMY_CELLS)

    local chosen = crawler_pick_distinct(encounter_table.crawler_pool, enemy_count)

    local enemies = {}

    for _, enemy in ipairs(chosen) do
        enemies[#enemies + 1] = { name = enemy.name, rank = enemy.rank, }
    end
...
ezencounters.increment_steps_since_encounter = function (player_id)
    ...
    local player_area = Net.get_player_area(player_id)
    local encounter_table = area_encounter_tables[player_area] or
...
ezencounters.try_random_encounter = function(player_id, encounter_table)
        ...
        if encounter_table.crawler_random then
            ...
            encounter_info = build_crawler_random_encounter(player_area, encounter_table)
        else
    ...
    if players_in_encounters[player_id] then
        ...
        if #rewards > 0 then
            if reward_delay_ticks and reward_delay_ticks > 0 then
                ...
                if recovery_reward then
                    recovery = math.max(0, math.floor(tonumber(recovery_reward.value) or 0))
                end
            else
                recovery = send_battle_rewards(player_id, rewards, event, player_encounter.persistent_health)
            end
        end
        ...
        if post_send_chip_key then
            queue_post_reward_unlock(player_id, post_send_chip_key)
        end
...
object_registry.register_handler("Radius Encounter", function(area_id, object)
    ...
    emitter:on('entered', function(event) return on_radius_encounter_triggered(event) end)
end)
