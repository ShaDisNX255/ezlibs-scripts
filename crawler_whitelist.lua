...
function crawler_whitelist.restore_unlocked_cards_for_current_run(
    ...
    local stored_run_id =
        player_memory.meta
        and player_memory.meta.crawler_run_id

    if tostring(stored_run_id or "")
        ~= tostring(run_id)
    then
        return false,
            "run_mismatch"
    ...
    -- --------------------------------------------------------
    -- STEP 1:
    -- Re-provide every chip asset earned in this run.
    -- --------------------------------------------------------

    local restored_cards = {}
        ...
        if unlocked then
            ...
            if card_def then
                ...
                if not provided then
                    print(
                        "[crawler_whitelist] failed providing restored chip " ..
                        tostring(chip_key) ..
                        ": " ..
                        tostring(provide_error)
                    )
                else
                    restored_cards[
                        #restored_cards + 1
    ...
    -- --------------------------------------------------------
    -- STEP 2:
    -- Rebuild and re-apply the whitelist AFTER all restored
    -- chip assets have been provided to this new connection.
    -- --------------------------------------------------------
    ...
    print(
        "[crawler_whitelist] re-authorized " ..
        tostring(
            #restored_cards
        ) ..
        " unlocked chip(s) for run " ..
    ...
    -- --------------------------------------------------------
    -- STEP 3:
    -- Give the client time to receive the assets + whitelist
    -- before the restored battle chips are sent.
    -- --------------------------------------------------------
