...
local crawler_loot_tiers = require("scripts/ezlibs-scripts/crawler_loot_tiers")
...
for card_key, card_def in pairs(crawler_whitelist.CARDS) do
    card_def.card_key = card_key

    local package_id = card_def.package_id

    if package_id and package_id ~= "" then
        card_key_by_package_id[package_id] = card_key

        locked_by_default[package_id] = true
    end
end
...
-- FILE HELPERS
-- ============================================================
local function get_player_reward_tier(player_id)
    ...
    local tier = Net.get_area_custom_property(area_id, "dungeon_reward_tier")

    if tier == "easy" or tier == "medium" or tier == "hard" then
        return tier
    end
...
local function build_card_reward_entry(card_def, code)
    if not card_def or not card_def.package_id or card_def.package_id == "" then
        return nil
    end

    return { type = 1, card_id = card_def.package_id, code = code or card_def.code or "*", }
end

local function queue_reward_packet(player_id, rewards, ticks)
    if not rewards or #rewards == 0 then
        return false
    end

    local delay = math.max(1, math.floor(tonumber(ticks) or POST_UNLOCK_REWARD_DELAY_TICKS))

    local pending = pending_reward_packets[player_id]
    ...
    if pending then
        ...
        pending_reward_packets[player_id] = { ticks = delay, rewards = rewards, }
    end
...
local function get_player_memory(player_id)
...
local function get_current_run_unlocks(player_id)
    local player_memory, safe_secret = get_player_memory(player_id)
    ...
    local area_run_id = Net.get_area_custom_property(area_id, "dungeon_run_id")

    if not area_run_id or tostring(area_run_id) == "" or tostring(area_run_id) == "pool" then
        -- Outside a dungeon run, earned run chips are locked.
        return {}, safe_secret, false
    end

    local stored_run_id = player_memory.meta and
        player_memory.meta.crawler_run_id

    if tostring(stored_run_id or "") ~= tostring(area_run_id) then
        return {}, safe_secret, false
    end

    if type(player_memory.crawler_chip_unlocks) ~= "table" then
...
function crawler_whitelist.begin_run_for_player(player_id, run_id)
    run_id = tostring(run_id or "")
    ...
    if not player_memory
        or not safe_secret
    then
        return false, "missing_player_memory"
    end

    if type(player_memory.meta) ~= "table" then
        player_memory.meta = {}
    end

    local previous_run_id = tostring(player_memory.meta.crawler_run_id or "")
    ...
    if previous_run_id ~= run_id then
        ...
        pending_reward_packets[player_id] = nil

        hydrated_run_for_player[player_id] = nil
    end
    ...
    player_memory.meta.crawler_run_id = run_id

    ezmemory.save_player_memory(safe_secret)

    print("[crawler_whitelist] player " .. tostring(player_id) .. " entered crawler run " .. run_id)
...
local function build_player_whitelist_text(player_id)
    local unlocks = select(1, get_current_run_unlocks(player_id))

    local base_text = read_text_file(BASE_WHITELIST_DISK_PATH)

    if not base_text then
        print("[crawler_whitelist] could not read " .. BASE_WHITELIST_DISK_PATH)
    ...
    for line in (base_text .. "\n"):gmatch("(.-)\n") do
        ...
        elseif not locked_by_default[package_id] then
            -- Ordinary whitelist entry:
            -- always allowed.
            out_lines[#out_lines + 1] = line

        else
            local card_key = card_key_by_package_id[package_id]

            if card_key and unlocks[card_key] == true then
...
function crawler_whitelist.apply_for_player(player_id)
    local safe_secret = helpers.get_safe_player_secret(player_id)
    ...
    if text then
        print("[crawler_whitelist][DEBUG] generated whitelist for player " .. tostring(player_id))
    end
    ...
    if not cached or cached.text ~= text or cached.path ~= asset_path then
        ...
        cached_whitelist[safe_secret] = { text = text, path = asset_path, }
    end
...
function crawler_whitelist.get_card_def(card_key)
    if not card_key then
        return nil
    end

    return crawler_whitelist.CARDS[tostring(card_key)]
end

function crawler_whitelist.card_allows_source(card_key, source)
    local card_def = crawler_whitelist.get_card_def(card_key)

    if not card_def or type(card_def.sources) ~= "table" then
        return false
    end

    return
        card_def.sources[source] == true
end

function crawler_whitelist.player_has_card_unlocked(player_id, card_key)
...
local function provide_card_asset(player_id, card_def)
    ...
    local hint = { asset_type = AssetType.DATA, package_type = PackageType.CARD, }

    local ok, err = pcall(Net.provide_asset_for_player, player_id, card_def.asset_path, hint)

    if not ok then
        print("[crawler_whitelist] warning: could not provide card asset: " .. tostring(err))
...
local function queue_unlocked_cards_for_current_run(player_id)
    ...
    local run_id = Net.get_area_custom_property(area_id, "dungeon_run_id")

    if not run_id or tostring(run_id) == "" or tostring(run_id) == "pool" then
        return false
    end
    ...
    if hydrated_run_for_player[player_id] == run_id then
        return false
    end

    local rewards = {}

    for card_key, is_unlocked in pairs(unlocks) do
        if is_unlocked == true then
            local card_def = crawler_whitelist.CARDS[card_key]

            if card_def then
                -- Re-send the package itself because the
                -- connecting client may not have it locally.
                provide_card_asset(player_id, card_def)

                local reward = build_card_reward_entry(card_def, card_def.code)
    ...
    hydrated_run_for_player[player_id] = run_id
    ...
    if #rewards > 0 then
        ...
        queue_reward_packet(player_id, rewards, REJOIN_REWARD_DELAY_TICKS)
    end

    return true
end

function crawler_whitelist.restore_unlocked_cards_for_current_run(player_id)
    crawler_whitelist.apply_for_player(player_id)

    return queue_unlocked_cards_for_current_run(player_id)
end

function crawler_whitelist.prepare_card_for_battle_reward(player_id, card_key, code)
    card_key = tostring(card_key or "")

    local card_def = crawler_whitelist.CARDS[card_key]

    if not card_def then
        return false, "unknown_card", nil, nil
    end

    local unlocks, _, active_run = get_current_run_unlocks(player_id)

    if not active_run then
        return false, "not_in_active_run", card_def, nil
    end

    if unlocks[card_key] then
        return false, "already_unlocked", card_def, nil
    end

    local reward = build_card_reward_entry(card_def, code or card_def.code)

    if not reward then
        return false, "invalid_reward", card_def, nil
    end

    print("[crawler_whitelist] prepared battle reward " .. card_key .. " for player " .. tostring(player_id))

    return true, "ready", card_def, reward
end

function crawler_whitelist.commit_card_after_battle_reward(player_id, card_key)
    card_key = tostring(card_key or "")

    local card_def = crawler_whitelist.CARDS[card_key]

    if not card_def then
        return false, "unknown_card", nil
    end

    local unlocks, safe_secret, active_run = get_current_run_unlocks(player_id)

    if not active_run then
        return false, "not_in_active_run", card_def
    end
    ...
    print("[crawler_whitelist] committed battle reward " .. card_key .. " for player " .. tostring(player_id))

    return true, "unlocked", card_def
end

function crawler_whitelist.unlock_card(player_id, card_key)
    card_key = tostring(card_key or "")
    ...
    if reward then
        queue_reward_packet(player_id, { reward }, POST_UNLOCK_REWARD_DELAY_TICKS)
    end

    print("[crawler_whitelist] unlocked " .. card_key .. " for player " .. tostring(player_id))

    return true, "unlocked", card_def
end

function crawler_whitelist.unlock_card_from_source(player_id, card_key, source)
    if not crawler_whitelist.card_allows_source(card_key, source) then
        return false, "source_not_allowed", crawler_whitelist.get_card_def(card_key)
    end

    return crawler_whitelist.unlock_card(player_id, card_key)
end

function crawler_whitelist.get_available_cards_for_source(player_id, source)
    local unlocks, _, active_run = get_current_run_unlocks(player_id)
    ...
    local reward_tier = get_player_reward_tier(player_id)

    local candidates = {}

    for card_key, card_def in pairs(crawler_whitelist.CARDS) do
        local tier_allowed = true
        ...
        if reward_tier and (source == "blue_mystery" or source == "chip_seller") then
            tier_allowed = crawler_loot_tiers.matches(source, card_key, reward_tier)
        end
        ...
        if
            ...
            tier_allowed
        then
            candidates[#candidates + 1] = card_key
        end
    end
...
function crawler_whitelist.unlock_random_card_from_source(player_id, source)
    local candidates = crawler_whitelist.get_available_cards_for_source(player_id, source)

    if #candidates == 0 then
        return false, "pool_exhausted", nil
    end

    local card_key = candidates[math.random(1, #candidates)]

    local ok, reason, card_def = crawler_whitelist.unlock_card_from_source(player_id, card_key, source)
...
function crawler_whitelist.handle_player_join(player_id)
    pending_reward_packets[player_id] = nil
    ...
    queue_unlocked_cards_for_current_run(player_id)
end

function crawler_whitelist.handle_player_transfer(player_id)
    crawler_whitelist.apply_for_player(player_id)
...
function crawler_whitelist.on_tick(delta_time)
    for player_id, packet in pairs(pending_reward_packets) do
        packet.ticks = packet.ticks - 1

        if packet.ticks <= 0 then
            local ok, err = pcall(Net.send_player_battle_rewards, player_id, packet.rewards)

            if not ok then
                print("[crawler_whitelist] failed sending chip rewards: " .. tostring(err))
            end
...
function crawler_whitelist.handle_player_disconnect(player_id)
    pending_reward_packets[player_id] = nil
